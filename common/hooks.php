...
function meta_auth_sync_data_with_server()
{
    ...
    if ($sessions) {
        $auth_token = AdAstraCrypto\WP\Api::getAuthToken(AUTH_PLUGIN);
        $ticker = sanitize_text_field($_POST['ticker']);
        set_time_limit(120);
        foreach ($sessions as $session) {
            $resp = AdAstraCrypto\WP\Api::request('/v1/data', 'PUT', [
                'ip' => $session->ip,
                'email' => $session->email,
...
{
    AdAstraCrypto\WP\Api::registerRoutes('meta-auth/v1');
}
...
function meta_auth_on_login()
{
    ...
    $ticker = sanitize_text_field($_POST['ticker']);
    $auth_token = AdAstraCrypto\WP\Api::getAuthToken(AUTH_PLUGIN);
    $resp = AdAstraCrypto\WP\Api::request("/v2/wallet-auth/nonce?address=$address&ticker=$ticker", 'GET', [], $auth_token);
    ...
    if ($resp) {
        ...
        $address = sanitize_text_field($_POST['account']);
        $auth_token = AdAstraCrypto\WP\Api::getAuthToken(AUTH_PLUGIN);
        $resp = AdAstraCrypto\WP\Api::request("/v2/wallet-auth/nonce?address=$address&ticker=ETH", 'GET', [], $auth_token);
...
function meta_auth_skip_wallet()
{
    ...
    $resp = AdAstraCrypto\WP\Api::request('/v3/data/wallet-skip', 'PUT', [
...
function meta_auth_on_verify()
{
    ...
    $resp = AdAstraCrypto\WP\Api::request('/v3/data', 'PUT', [
        'wallet' => $account,
        'ticker' => $ticker,
        'balance' => $balance,
        'data' => [
